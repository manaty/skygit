...
async function updateRepoMessagingConfig(token2, repo) {
  ...
  await commitRepoToGitHub2(token2, repo);
}
...
const githubApi = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
...
const conversationCommitQueue = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  flushConversationCommitQueue,
...
var root_2$4 = /* @__PURE__ */ template(`<p class="text-xs text-gray-400 italic truncate mt-1"> </p>`);
var root_3$5 = /* @__PURE__ */ template(`<p class="text-xs text-gray-300 italic mt-1">No messages yet.</p>`);
var root_1$6 = /* @__PURE__ */ template(`<button class="px-3 py-2 hover:bg-blue-50 rounded cursor-pointer text-left"><p class="text-sm font-medium truncate"> </p> <p class="text-xs text-gray-500 truncate"> </p> <!></button>`);
var root_4$2 = /* @__PURE__ */ template(`<p class="text-xs text-gray-400 italic px-3 py-4">No conversations yet.</p>`);
...
function SidebarChats($$anchor, $$props) {
  ...
  each(node, 1, () => get$1(allConversations), (convo) => convo.id, ($$anchor2, convo) => {
    ...
    var node_1 = sibling(p_1, 2);
    {
      ...
      if_block(node_1, ($$render) => {
        if (get$1(convo).messages && get$1(convo).messages.length > 0) $$render(consequent);
        else $$render(alternate, false);
      });
    }
    template_effect(
  ...
  var node_2 = sibling(node, 2);
  {
    ...
    if_block(node_2, ($$render) => {
      if (get$1(allConversations).length === 0) $$render(consequent_1);
    });
  }
  append($$anchor, div);
  pop();
...
async function getOrCreatePresenceDiscussion(token2, repoFullName2) {
  var _a2;
  const headers2 = {
    Authorization: `token ${token2}`,
  ...
  let discussions = [];
  try {
    const res = await fetch(discussionsUrl, { headers: headers2 });
    if (res.ok) {
...
async function postPresenceComment(token2, repoFullName2, username, sessionId2, signaling_info = null) {
  ...
  const myComment = comments.find((c) => {
    try {
      ...
      return body.username === username && body.session_id === sessionId2;
    } catch (e) {
      return false;
    }
  });
  ...
  for (const c of comments) {
    try {
      const body = JSON.parse(c.body);
      if (body.username === username && body.session_id !== sessionId2) {
...
async function markPeerForPendingRemoval(token2, repoFullName2, peerUsername, peerSessionId, removerUsername) {
  const discussionNumber = await getOrCreatePresenceDiscussion(token2, repoFullName2);
  const headers2 = {
    Authorization: `token ${token2}`,
    Accept: "application/vnd.github+json, application/vnd.github.squirrel-girl-preview+json"
...
async function cleanupStalePeerPresence(token2, repoFullName2, peerUsername, peerSessionId) {
  const discussionNumber = await getOrCreatePresenceDiscussion(token2, repoFullName2);
  const headers2 = {
    Authorization: `token ${token2}`,
    Accept: "application/vnd.github+json, application/vnd.github.squirrel-girl-preview+json"
...
async function pollPresenceFromDiscussion(token2, repoFullName2) {
  const discussionNumber = await getOrCreatePresenceDiscussion(token2, repoFullName2);
  const headers2 = {
    Authorization: `token ${token2}`,
    Accept: "application/vnd.github+json, application/vnd.github.squirrel-girl-preview+json"
...
async function postHeartbeat(token2, repoFullName2, username, sessionId2, signaling_info = null) {
  await postPresenceComment(token2, repoFullName2, username, sessionId2, signaling_info);
}
async function pollPresence(token2, repoFullName2) {
  return await pollPresenceFromDiscussion(token2, repoFullName2);
}
class SkyGitWebRTC {
...
function getCurrentLeader(peers, localUsername2) {
  const usernames = peers.map((p) => p.username).concat(localUsername2);
  return usernames.sort()[0];
}
function isLeader(peers, localUsername2) {
  return getCurrentLeader(peers, localUsername2) === localUsername2;
}
...
function handleSignalMessage(msg, fromUsername) {
  ...
  peerConnections.update((conns) => {
    var _a2;
    const peer = (_a2 = conns[fromUsername]) == null ? void 0 : _a2.conn;
    ...
    if (!leaderCommitInterval) {
      leaderCommitInterval = setInterval(() => {
        ...
        if (isLeader(currentPeers, localUsername)) {
          __vitePreload(() => Promise.resolve().then(() => conversationCommitQueue), true ? void 0 : void 0).then((mod) => {
...
function sendMessageToPeer(username, message) {
  peerConnections.update((conns) => {
...
var root_4$1 = /* @__PURE__ */ template(`<div class="flex flex-col h-full"><div class="flex items-center justify-between px-4 py-2 border-b"><div><h2 class="text-xl font-semibold"> </h2> <p class="text-sm text-gray-500"> </p></div> <div class="text-sm text-gray-500"> </div> <div class="ml-4 flex flex-wrap gap-3 items-center"><!> <!></div></div> <!> <!> <!> <!> <div class="flex-1 overflow-y-auto"><!></div> <div class="border-t p-4"><!></div></div>`);
...
function Chats($$anchor, $$props) {
  ...
  peerConnections.subscribe((update2) => {
    Object.keys(update2).filter((username) => update2[username].status === "connected").map((username) => ({ username }));
  });
  ...
  function cleanupPresence() {
    ...
    const username = ((_a2 = auth == null ? void 0 : auth.user) == null ? void 0 : _a2.login) || null;
    const repo = get$1(selectedConversation2) ? get$1(selectedConversation2).repo : null;
    const sessionId2 = window.skygitSessionId;
    if (token2 && username && repo && sessionId2) {
            ...
            var alternate = ($$anchor4) => {
                          ...
                          var consequent_3 = ($$anchor8) => {
                            ...
                            var node_6 = first_child(fragment_6);
                            {
                              var consequent_2 = ($$anchor9) => {
                                ...
                                template_effect(() => set_text(text_3, ` ${get$1(uname) ?? ""}`));
                          ...
                          var alternate_3 = ($$anchor8, $$elseif) => {
                            {
                              ...
                              var alternate_4 = ($$anchor9) => {
                                ...
                                var text_6 = sibling(child(span_4));
                ...
                var consequent_20 = ($$anchor5) => {
                  ...
                  var node_9 = child(span_5);
                  {
                    var consequent_7 = ($$anchor6) => {
                      var text_7 = text("🎤 Mic On");
                      append($$anchor6, text_7);
                    };
                    var alternate_5 = ($$anchor6) => {
                      var text_8 = text("🔇 Mic Off");
                  ...
                  var node_10 = child(span_6);
                  {
                    var consequent_8 = ($$anchor6) => {
                      var text_9 = text("📷 Cam On");
                      append($$anchor6, text_9);
                    };
                    var alternate_6 = ($$anchor6) => {
                      var text_10 = text("🚫📷 Cam Off");
                  ...
                  var node_11 = child(span_7);
                  {
                    var consequent_9 = ($$anchor6) => {
                      var text_11 = text("🎤 Mic On");
                      append($$anchor6, text_11);
                    };
                    var alternate_7 = ($$anchor6) => {
                      var text_12 = text("🔇 Mic Off");
                  ...
                  var node_12 = child(span_8);
                  {
                    var consequent_10 = ($$anchor6) => {
                      var text_13 = text("📷 Cam On");
                      append($$anchor6, text_13);
                    };
                    var alternate_8 = ($$anchor6) => {
                      var text_14 = text("🚫📷 Cam Off");
                  ...
                  var node_15 = child(button_3);
                  {
                    var consequent_13 = ($$anchor6) => {
                      var text_16 = text("🛑 Stop Sharing");
                      append($$anchor6, text_16);
                    };
                    var alternate_9 = ($$anchor6) => {
                      var text_17 = text("🖥️ Share Screen");
                  ...
                  var node_16 = sibling(button_3, 2);
                  {
                    var consequent_14 = ($$anchor6) => {
                      var button_4 = root_29();
                      event("click", button_4, changeScreenSource);
                  ...
                  template_effect(() => {
                    set_attribute(button_5, "title", get$1(micOn) ? "Mute Mic" : "Unmute Mic");
                    set_attribute(button_6, "title", get$1(cameraOn) ? "Turn Off Camera" : "Turn On Camera");
                    set_attribute(button_7, "title", get$1(recording) ? "Stop Recording" : "Start Recording");
                  });
                  ...
                  event("click", button_5, toggleMic);
                  event("click", button_6, toggleCamera);
                ...
                var consequent_22 = ($$anchor5) => {
                  ...
                  var node_23 = first_child(fragment_8);
                  {
                    var consequent_21 = ($$anchor6) => {
                      ...
                      var video_2 = sibling(button_8, 4);
                      video_2.muted = true;
                      ...
                      event("click", button_8, stopPropagation(closePreview));
                      event("mousedown", div_17, onPreviewMouseDown);
                      append($$anchor6, div_17);
                    };
                    var alternate_13 = ($$anchor6) => {
                      var button_9 = root_40();
                      event("click", button_9, reopenPreview);
              ...
              var node_24 = sibling(node_22, 2);
              {
                var consequent_23 = ($$anchor5) => {
                  ...
                  var button_10 = sibling(child(div_20), 2);
                  var button_11 = sibling(button_10, 2);
                  var button_12 = sibling(button_11, 2);
                  var button_13 = sibling(button_12, 2);
                  event("click", button_10, () => selectShareType("screen"));
                  event("click", button_11, () => selectShareType("window"));
                  event("click", button_12, () => selectShareType("tab"));
                  event("click", button_13, closeShareTypeModal);
              ...
              var node_25 = sibling(node_24, 2);
              {
                var consequent_24 = ($$anchor5) => {
                  ...
                  var button_14 = sibling(child(div_22), 2);
                  ...
                  event("click", button_16, resetUploadDestination);
              ...
              template_effect(() => {
                ...
                set_text(text_1, get$1(selectedConversation2).repo);
                set_text(text_2, `${((_a2 = get$1(selectedConversation2).participants) == null ? void 0 : _a2.length) ?? 0} participants`);
              });
              append($$anchor4, div_3);
            };
            if_block(node_1, ($$render) => {
