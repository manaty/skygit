...
import { onlinePeers, broadcastMessage, getLocalSessionId, broadcastTypingStatus } from '../services/peerJsManager.js';
    ...
    import { Paperclip, Loader2, X } from 'lucide-svelte';
    ...
    $: hasStorageConfigured = repo?.config?.binary_storage_type && repo?.config?.storage_info?.url;
  </script>
  ...
  <div class="space-y-2">
    ...
    {#if selectedFile}
      <div class="bg-blue-50 px-3 py-2 rounded text-sm flex items-center justify-between">
